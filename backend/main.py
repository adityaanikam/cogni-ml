...
class LoanRequest(BaseModel):
    ...
    def validate_cibil_id(cls, v):
        ...
        if isinstance(v, str):
            v = v.strip()
            if not v.isdigit():
                raise ValueError('CIBIL ID must contain only digits')
            if len(v) != 9:
                raise ValueError('CIBIL ID must be exactly 9 digits')
        return v
...
async def predict_loan(request: LoanRequest):
    ...
    if cibil_score is None:
        raise HTTPException(status_code=404, detail=f"CIBIL ID '{request.cibil_id}' not found.")
